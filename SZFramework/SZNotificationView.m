...
@implementation SZNotificationView
                    ...
                    duration:(NSTimeInterval)duration
{
    ...
    __block SZNotificationView* note = [[SZNotificationView alloc] initWithParentViewController:viewController];
                     ...
                     message:(NSString *)message
{
    ...
    [SZNotificationView showInViewController:viewController
                                   tintColor:[SZNotificationView blurTintColorForStyle:style]
                                       image:[SZNotificationView imageForStyle:style]
                                     message:message
                                    duration:kSZNotificationViewDefaultShowDuration];
}
                                                        ...
                                                        message:(NSString*)message
{
    ...
    SZNotificationView* note = [[SZNotificationView alloc] initWithParentViewController:viewController];
    note.tintColor = tintColor;
    note.image = image;
    note.textLabel.text = message;
...
- (instancetype)initWithParentViewController:(UIViewController*)viewController
{
    ...
    if (self) {
        ...
        //Notifications
        {
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(navigationControllerWillShowViewControllerNotification:) name:kSZNotificationViewUINavigationControllerWillShowViewControllerNotification object:nil];
        }
...
- (void)updateConstraints
{
    ...
    CGFloat symbolViewWidth = self.symbolView.tag != kSZNotificationViewEmptySymbolViewTag ?
    kSZNotificationViewSymbolViewSidelength : 0.0f;
    CGFloat symbolViewHeight = kSZNotificationViewSymbolViewSidelength;
...
- (CGRect)visibleFrame
{
    ...
    CGRect displayFrame = CGRectMake(0, 0, fabs(transformedSize.width),
                                     kSZNotificationViewHeight + topLayoutGuideLength);
...
- (CGRect)hiddenFrame
{
    ...
    CGSize transformedSize = CGSizeApplyAffineTransform(viewController.view.frame.size, viewController.view.transform);
    CGRect offscreenFrame = CGRectMake(0, -kSZNotificationViewHeight - topLayoutGuideLength,
...
- (void)updateSymbolView
{
    ...
    if (self.isShowingActivity) {
        ...
        _symbolView.tag = kSZNotificationViewEmptySymbolViewTag;
    }
...
+ (UIImage*)imageForStyle:(SZNotificationViewStyle)style
{
    UIImage* matchedImage = nil;
    switch (style) {
        case SZNotificationViewStyleSuccess:
            matchedImage = [UIImage imageNamed:@"SZNotificationView_checkmarkIcon"];
            break;
        case SZNotificationViewStyleError:
            matchedImage = [UIImage imageNamed:@"SZNotificationView_exclamationMarkIcon"];
            break;
        default:
            break;
    }
...
+ (UIColor*)blurTintColorForStyle:(SZNotificationViewStyle)style
{
